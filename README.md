...
## One command: video to a live 3D website

This unified repository includes the original model source plus a complete,
token-protected web deployment. On a CUDA host with Docker and the
`docker gpu` broker installed, run:

```bash
git clone https://github.com/robit-man/lingbot-map-oneclick.git
cd lingbot-map-oneclick
./run.sh
```

That single command creates a local bootstrap venv, installs the checkpoint
client, downloads and verifies the pinned 4.63 GB model, builds the minified web
app and CUDA image, installs Cloudflared, acquires one exact broker-scoped GPU,
starts a supervised service, and prints the public URL and access token.

Open the URL, paste the token, choose or drop one video, and click **Build 3D
map**. Upload, frame extraction, GPU reconstruction, and browser-model export
are shown as live progress; the finished map opens directly in the orbitable
Three.js viewer and can also be downloaded as a GLB.

```bash
./run.sh status   # service, containers, and GPU lease
./run.sh logs     # live application/tunnel output
./run.sh stop     # preserves checkpoints and results
```
...
- [🌐 One-command Web Deployment](#-one-command-web-deployment)
...
For an automatic checkpoint pull, minified upload-and-view web app, scoped CUDA
deployment, and Cloudflare tunnel, use the included supervised deployment:

```bash
./run.sh
```

See [DEPLOYMENT.md](DEPLOYMENT.md) for checkpoint integrity pins, named-tunnel
configuration, GPU broker behavior, security limits, and verification commands.
